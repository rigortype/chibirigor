...
| 2 | 型理論エキスパート | 形式的・技術的正確さ | `book/v1/ja/_expert-review.md` |
| 3 | 技術書編集者 | 構成・学習設計・刊行完成度 | `book/v1/ja/_editorial-review.md` |
| 4 | ドメイン著者（mametter） | 概念の位置づけ・自著引用の公正さ・TypeProf 対比 | `book/v1/ja/_mametter-review.md` |
| 5 | 日本語校正・校閲 | 言語の質（慣用句誤用・AI 調・表記ゆれ） | `book/v1/ja/_copyedit-review.md` |
| 6 | Rigor フィデリティ | 「実 Rigor では…」の記述が実態と一致するか | `book/v1/ja/_fidelity-review.md` |
| 7 | Java 中堅読者（説明の飛躍） | 型理論ゼロ・静的型付け経験ありの読者に飛躍がないか | `book/v1/ja/_java-reader-review.md` |
| 8 | Ruby/Rails ジュニア読者 | 型宣言のメンタルモデルが無い Ruby 読者に通じるか | `book/v1/ja/_ruby-reader-review.md` |
| 9 | 書評家（読み物としての質） | 散文・背景の厚み・関連情報の織り込み | `book/v1/ja/_book-review.md` |
| 10 | 辛口書評家（皮肉屋・批判的読み） | #9 と同属性で逆張り。誇張・空疎・はったり・約束違反を抉る | `book/v1/ja/_harsh-review.md` |
...
## 起動 → 統合 → 適用
1. 依頼に応じてレンズを選ぶ（既定＝全 10。引数で `copyedit` / `fidelity` / `java-reader` / `ruby-reader` / `book-review` / `harsh-review` 等の単独も可）。独立なので
   **同一メッセージで並列起動**。
2. 各レンズの所見を **`book/v1/ja/_<lens>-review.md` にコミット**（永続・非同期の台帳）。
3. **統合と選択適用**：
...
4. 結果を `book/v1/ja/_handoff-state.md` のバックログに反映。

## 注意
- レンズは**節目で**回す（per-edit ではない）。全 10 は重いので、軽い点検は該当レンズだけ。
