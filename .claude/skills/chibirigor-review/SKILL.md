...
| 2 | 型理論エキスパート | **L1 真** | 形式的・技術的正確さ | `book/v1/ja/.reviews/_expert-review.md` |
| 3 | 技術書編集者 | **L4 整** | 構成・学習設計・刊行完成度 | `book/v1/ja/.reviews/_editorial-review.md` |
| 4 | ドメイン著者（mametter） | **L1 真** | 概念の位置づけ・自著引用の公正さ・TypeProf 対比 | `book/v1/ja/.reviews/_mametter-review.md` |
| 5 | 日本語校正・校閲 | **L4 整** | 言語の質（慣用句誤用・AI 調・表記ゆれ） | `book/v1/ja/.reviews/_copyedit-review.md` |
| 6 | Rigor フィデリティ | **L1 真** | 「実 Rigor では…」の記述が実態と一致するか | `book/v1/ja/.reviews/_fidelity-review.md` |
| 7 | Java 中堅読者（説明の飛躍） | **L2 伝** | 型理論ゼロ・静的型付け経験ありの読者に飛躍がないか | `book/v1/ja/.reviews/_java-reader-review.md` |
| 8 | Ruby/Rails ジュニア読者 | **L2 伝** | 型宣言のメンタルモデルが無い Ruby 読者に通じるか | `book/v1/ja/.reviews/_ruby-reader-review.md` |
| 9 | 書評家（読み物としての質） | **L3 読** | 散文・背景の厚み・関連情報の織り込み | `book/v1/ja/.reviews/_book-review.md` |
| 10 | 辛口書評家（皮肉屋・批判的読み） | **L3 読** | #9 と同属性で逆張り。誇張・空疎・はったり・約束違反を抉る | `book/v1/ja/.reviews/_harsh-review.md` |
...
- 注意：再現性レンズ（型理論ゼロだが*実装して*採点）とは別物。こちらは **「読んで理解が繋がるか・
  ...
  nitpick）を付ける。出力ノート `book/v1/ja/.reviews/_java-reader-review.md`。
...
- 探す：型≒クラスの素朴像との未接続、型注釈/静的型を既知扱いした飛躍、TS/他記法の足場化、未定義の
  ...
  *それも無い*）点に注意。出力ノート `book/v1/ja/.reviews/_ruby-reader-review.md`。
...
- 探す：背景・動機・由来の欠落（「天下り的に手順だけ」）、羅列に散文が伴わない箇所、関連分野への
  ...
  **羅列的**＝散文の接続線が要る所／nitpick）。出力ノート `book/v1/ja/.reviews/_book-review.md`。
...
- 探す：盛った表現・自賛、衒学的参照、滑った比喩、約束と中身の落差、著者のための饒舌、軽佻な口調。
  ...
  `book/v1/ja/.reviews/_harsh-review.md`。
...
2. **レイヤーを 1 つ回す**：そのレイヤーのレンズを**同一メッセージで並列起動**。各所見を
   **`book/v1/ja/.reviews/_<lens>-review.md` にコミット**（永続・非同期の台帳）。
3. **そのレイヤー分を統合・選択適用**：
