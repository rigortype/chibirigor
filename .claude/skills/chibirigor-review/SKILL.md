...
## 起動 → 統合 → 適用
1. 依頼に応じてレンズを選ぶ（既定＝全 8。引数で `copyedit` / `fidelity` / `java-reader` / `ruby-reader` 等の単独も可）。独立なので
   **同一メッセージで並列起動**。
...
## 注意
- レンズは**節目で**回す（per-edit ではない）。全 8 は重いので、軽い点検は該当レンズだけ。
