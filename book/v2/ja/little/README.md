...
最小の Ruby 型チェッカー `chibirigor` を、ステップバイステップで**作りながら**学ぶ前編です。
`check`（型診断）と `annotate`（推論した型の表示）が動く最小実装を Part 0〜9 で作り切ります。
**コードは前編だけで動き切ります**。概念の深掘り（形式的な理論）は後編へ譲ります。
...
| [4](part4-union.md) | Union ― 型が一本に決まらない |
| [5](part5-narrowing.md) | ナローイング ― 場合分けで絞る |
...
読み終えたら、後編 [The Seasoned chibirigor](../seasoned/README.md)（**読み解く**巻）へ続きます。
