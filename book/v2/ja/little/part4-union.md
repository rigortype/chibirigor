...
> [!NOTE]
> 私たちが作るUnion（`Integer | String`のような**無タグのユニオン型**）は、実は参考書があえて避けた出発点です。『しくみ』も『TAPL』も、持っているのは値に札を付けて区別するタグ付きのvariantであって、無タグのUnionとは別物です。でもRubyを相手にする私たちには、無タグのUnionが必須です。（この違いは付録[a5-4](../appendix/a5-other-languages.md)へ。）
