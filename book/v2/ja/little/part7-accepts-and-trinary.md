...
> 同書は「合う／合わない」を`true`/`false`の二択で答えました。
> 私たちはそこに一つ足します。
...
`type_of`は、わからない式に対して`Dynamic`（untyped）を返すのでした。Part 1でそう決めました。
では：
...
この「箱に入るか」が、『しくみ』 7章の部分型付けそのものです。用語は覚えなくて大丈夫です。*小さい箱は大きい箱に入る*、とだけ押さえておいてください。
...
> `accepts`はその入口です。Javaの継承／GoやTypeScriptの構造的な型との対応は付録[a5-2](../appendix/a5-other-languages.md)へ。形式の扱いは後編Part 2で確認します。
...
これで`1 + (c ? 1 : 2)`のようにUnionが引数に来ても、中身が全部`Integer`なら`:yes`で黙ります。誤検知しません。
