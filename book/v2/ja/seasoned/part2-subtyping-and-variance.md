...
> （`Top`／`Bot`と`untyped`／`void`／`never`の関係は付録[a1](../appendix/a1-special-types.md)「特別な型カタログ」へ。）
...
`Arrow`の行の`subtype(tp, sp)`（`sp`と`tp`が入れ替わっている）が、引数の反変そのものです。単体で走る設計スケッチ[`examples/subtype.rb`](examples/subtype.rb)で、`ruby subtype.rb`がこう**緑**になります（`arg contravariant reverse is false`が反変の証拠）：
...
前編の`accepts`は`:yes/:no/:maybe`の三値でした。`untyped`（`Dynamic`）が絡むと、`<:`は両方向に通る特別な関係（**gradual consistency（漸進的整合性）** `~`）になり、三値はこれを畳むための仕掛けでした。`Top`／`Bot`が「格子の両端の型」だったのに対し、`untyped`は「`<:`判定そのものを切る」点が決定的に違います（付録[a1](../appendix/a1-special-types.md)へ）。
