...
ここで余帰納の`seen`に当たるのが**fuel（燃料）予算**です。「再帰の展開をどう止めるか」という同じ問題に、『しくみ』と『TAPL』は*理論的*に（余帰納で正しく等価判定）、Rigorは*工学的*に（fuelで安全に打ち切り）答えています。**この「余帰納vs予算」という停止性の工学の総括は、健全性を扱う後編Part 7にまとめてあります**（gradualの割り切りとあわせて読み解きます）。
...
| Rigorの別解 | **HKTと`App`とfuel**（HKTの根拠は『TAPL』 29章）：余帰納の代わりに*燃料予算*で打ち切る（停止性の工学の総括はPart 7） |
