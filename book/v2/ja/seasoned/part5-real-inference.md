...
もう一段進めると、**型変数**を置いて**制約**を集め、まとめて解きます。これがTAPL 22章の**型再構築（type reconstruction）**、いわゆるHindley–Milnerの骨子です。
...
この「制約を集めて単一化で解く」核を、動く最小スケッチにしたのが[`examples/unification.rb`](examples/unification.rb)です。型変数`TVar`と基底型`TCon`だけの世界で、単一化はこれだけです：
