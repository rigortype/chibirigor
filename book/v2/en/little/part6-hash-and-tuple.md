...
- **① Type theory:** in record **width subtyping**, the side with *more* keys is the subtype
  ...
  - It can read backwards, but the relation is: "you can **pass** `{name:, admin:}` where
    `{name:}` is wanted (`name` is duly there); you can't pass the reverse."
  - The side with more keys passes more demands, so it's the subtype. "Subtype" is taken up in
    the next chapter, Part 7, as "does it fit in the box."
...
> [!NOTE]
> **`HashShape` is not Rigor's invention**
>
