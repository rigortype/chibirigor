...
> ほかに、ソースに `dump_type(式)` と書いて `check` する手があります（`include Rigor::Testing`
> が要ります）。`dump_type` は実行時には値をそのまま返すだけ ― `check` したときに**その位置の
...
推論型を `:info` で印字し、値はそのまま返します（だから `dump_type(x)` の型は `x` の型のまま）。
