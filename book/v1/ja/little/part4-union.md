...
> 私たちが作る Union（`Integer | String` のような**無タグのユニオン型**）は、実は参考書が*あえて
...
決めず「`Integer` か `String` のどちらか」という型にします。これが **Union** です：
...
> `type_of` に渡さない**点に注意してください（渡すと未知のノードとして `untyped` に落ちてしまいます）。
...
`x = cond ? 1 : 2` で `x` は `1 | 2` です。では `x + 1` の型は？ この章の最小版（と Part 2 の素朴な
...
組ごとに畳みます（`const_combinations`）― `1 + (1 | 2)` なら `1+1` と `1+2` を両方計算して `2 | 3` になります。
...
`x = cond ? 1 : "a"` の `x + 1` は、`1 + 1` は通り `"a" + 1` は型エラーです。でも実行時には `x` が
...
足したものは、型キャリア `Union` ひとつ、まとめ道具 `union`、そして `IfNode` の型付け
（then 節と else 節の型を `union` する）です。`union` の小道具は「入れ子をならす・重複を消す」の
2 つだけです。これで「型が一本に決まらない」Ruby を、型のレベルで素直に表せるようになりました。
...
`c ? 1 : "a"` は then 節が `1`・else 節が `"a"` です。どちらか一方に決めつけず `1 | "a"` という
Union にまとめます。else 側が `nil` でも同じく `1 | nil` です。

この章の三題噺：

| | 内容 |
|---|---|
| ① 型理論 | 値が複数の型になり得る＝ユニオン型（『しくみ』が*あえて避けた*領域。TAPL も直接の章なし） |
