...
> このメソッドは `Integer` を欲しがっている。あなたが渡したのは `String` だ。── **合う？**
...
こともあります。**渡す側（actual）が Union のときは、実際にどのメンバの値が来るか分からない**ので、
全メンバが通って初めて安全 ― だから **メンバを全部 `accepts` にかけて、一番弱い答えを採る**だけです。
...
`expected` が Union の場合（`accepts(Integer | String, Integer)` など）は**一番強い答え**を採ります ― どれか一つに合えば `:yes` です。「`Integer` か `String` のどちらかを期待」していて、渡されたのが `Integer` なら問題ない、という直感です。
...
`accepts` を `check` で使います。エラーを報告する場所は、**「欲しい型が決まっている所」** だけです。
...
足したものは、関数 `accepts`（`:yes`/`:no`/`:maybe` を返す）と、`check` がそれを使って
**`:no` のときだけ怒る**仕組みです。コードは `accepts` 本体 4 行＋Union のおまけ数行＋`check` の
差し替えだけです。新しい型キャリアは増えていません。
