...
> 実 Rigor には、指定位置の式の型を引く `rigor type-of` というコマンドがあります。chibirigor の
> `annotate` は内部の型をそのまま見せる最小版ですが、実ツールは「内部では精密に知り、RBS の境界では
> 粗く丸める」二重構造を持ちます。その仕組みは付録 [a3-2](../appendix/a3-tooling.md) へ。
