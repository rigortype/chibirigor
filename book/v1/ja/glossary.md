...
- **丸め／正規化（normalization）**〔前編 P1〕… 細かい型（`Const[3]`）を大ざっぱな型
  （`Integer`）に戻すこと。
- **拡大（widening）**〔前編 P2〕… `Const` のサイズが予算（閾値）を超えたら、際限なく抱え込まず
  大ざっぱな型へ移すこと。`Union` のメンバ数が上限を超えたときも同様（前編 P5）。抽象解釈の
  widening にあたり、「型も*予算を持つ*」設計の現れ。丸めが*つねに*粗くする正規化なのに対し、
  拡大は*予算超過時*の打ち切り。
...
- **erasure（境界での型の落とし込み）**〔前編 P1（予告：付録 a3-2）／後編 P3 で本式〕… Rigor 内部の精密な型
