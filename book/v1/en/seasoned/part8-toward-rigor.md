...
| Part | Little volume's minimal version | Rigor's real thing |
|---|---|---|
...
|---|---|---|
| **Plugin mechanism** | type Rails/RSpec/dry-rb DSLs/macros/open classes from outside without soiling the core | ADR-2 (extension API) → ADR-37 (interface separation) → ADR-9 (inter-plugin coordination) |
...
into it causes *core-soiling* problems — interference between tests, contamination by library-specific
