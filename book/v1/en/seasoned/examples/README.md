...
> **English edition note.** The English chapters print excerpts with **hand-translated English
> comments** and (currently) omit the `<!-- include: -->` / `<!-- run: -->` markers, since the shared
> `.rb` sketches carry Japanese comments and English-side example output is transcreated. Wiring
> `check_docs.rb` against `book/v1/en/` (and generating an English-commented sketch tree) is part of the
> open shared-tree decision — see the contributor guide [STYLE.md](../../STYLE.md) and the handoff
> ledger. Until then, the English example output is verified by reading.
