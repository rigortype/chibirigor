...
**型を自分で推論する、最小限の Ruby 型チェッカー**を、ステップバイステップで実装して学ぶ
オンラインブック／チュートリアル。
...
- **推論を土台にした検査器**：Ruby のコードにはふつう型注釈がないので、まず型を自分で
  導出します（推論の側）。そして導出した型を使って矛盾を見つけたり（`check`）、型を
  見せたり（`annotate`）します。成果物は診断（`check`）で、推論はそのエンジンです。
...
（ラムダノート）を各章で参照します。型理論 ↔ Ruby/RBS ↔ Rigor の実装、この三つを
やさしく結びつけるのがねらいです。また Benjamin C. Pierce
[『型システム入門 ― プログラミング言語と型の理論』](https://tatsu-zine.com/books/types-and-programming-languages-ja)
（オーム社）も随所で参照します。
...
本書（`draft/` 以下のテキストおよびドキュメント）は
[Creative Commons Attribution-ShareAlike 4.0 International](LICENSE)（CC BY-SA 4.0）の下で
公開しています。
