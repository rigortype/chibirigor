...
- **本文（チュートリアル）**：`draft/` 以下。二巻構成で前編 = **The Little chibirigor**
  （`draft/little/ja/`、全 10 章 Part 0〜9）、後編 = **The Seasoned chibirigor**
  （`draft/seasoned/ja/`、全 8 章 Part 1〜8。各章末に演習・図・動く設計スケッチ）。
- **設計・ロードマップ・副読本対応**：`docs/`（`ROADMAP.md` に本体の機能追加方針）。
- いずれも Rigor リポジトリには置かない（分離を維持するため）。

## 本文への機能反映（発展ノート方式）

本体（`lib/`）に機能を足したとき、**本文の「教えるためのコード」をその場で書き換えてはいけない。**
代わりに**文章（発展ノート）として重ねる**。学習設計（1 章 1 難所・まず一番シンプルに）を守るため。

- 各章の埋め込みコードは、その章の**いちばんシンプルな版**に保つ（最小ステップの語りを崩さない）。
- 後付けの機能は、章末近くに **prose の発展ノート**（例：`## N-Mx. 発展：◯◯` / `## N-Mb. ◯◯`）
  として足す。中身は **別建てのコード抜粋 ＋ その出力 ＋ 実装の置き場 ＋ FP の性質**。元コードの
  *差し替え*はしない。
- 章のコードの出力と*実物の道具*の挙動がズレる場合（例：章は「丸める」が `exe/chibirigor` は
  「畳む」）：章の出力は**章のコードの挙動**を示し、**橋渡しの一文**で「最終の道具はこうなる
  （発展参照）」と繋ぐ（手元で違う値が出る理由を明示する）。
- **先例**：ソース位置＝前編 Part 1「1-4b 診断を読みやすく」、定数畳み込み＝前編 Part 1
  「1-4c 発展：定数畳み込み」。どちらも元の `diagnostic`／`type_of_call` を据え置き、機能を
  発展ノートで重ねている。
- 理由：機能ごとに教えるコードを churn すると「まず一番シンプルに」という設計が崩れ、差分も
  読みにくい。発展ノート方式なら**本筋は据え置き・機能は積層・差分は局所**。
...
- **丸数字は `①②③…`（U+2460〜）を用いる**。`➀➁➂…`（U+2780〜のダインバット sans-serif）や
  `❶❷…`（U+2776〜の反転）は**使わない**。三題噺の ①型理論／②Ruby・RBS／③Rigor 実装、
  dispatch の段番号なども `①` 系で統一する。
...
- 章末の `> 検証メモ`（執筆者向け足場）は刊行時に除去（git 履歴に残る）。読者向けの**参考書メモ**
  （『しくみ』/TAPL）は残す。
- **framing の不可逆点（後退させない）**：chibirigor は「**推論を土台にした型チェッカー**」。やるのは
