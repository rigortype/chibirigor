...
- **高度な内容は続編へ**：双方向型付けの形式化・変性・再帰型・本物の型推論・健全性理論は
  「chibirigor 修行」に送る（本編に持ち込まない）。

## アーキテクチャ（最小版）

`lib/chibirigor/` … `type.rb`（型カリア）/ `type_of.rb`（式→型の合成）/
`dispatch.rb`（メソッド表）/ `scope.rb` / `narrow.rb` / `accepts.rb` / `checker.rb` /
`annotator.rb`、`exe/chibirigor`（CLI）。各 Part が 1 ファイルを足す/育てる。
...
設計・本文（チュートリアル）・副読本対応は**すべて `docs/`** に置く。Rigor リポジトリには
置かない（分離を維持するため）。
