...
**同じペアを再び問われたら「等しい」と仮定して打ち切る**：

```
typeEq(S, T, seen):
  if (S, T) ∈ seen:  return true          # 証明中のペア → 等しいと仮定（打ち切り）
  if S is μ:         return typeEq(unfold(S), T, seen ∪ {(S, T)})
  if T is μ:         return typeEq(S, unfold(T), seen ∪ {(S, T)})
  else:              S と T を構造的に比較（seen を引き回す）
```
