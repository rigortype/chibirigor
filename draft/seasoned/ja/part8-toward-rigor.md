...
| **baseline／オンボーディング** | 既存コードへの後付け導入（前編 P9 の baseline の本式） | ADR-22（baseline ＋ オンボーディング） |
...
- 「再帰・推論をどう止めるか」← 後編 Part 4（fuel）・Part 7（予算）。**読むなら**
  ADR-20（軽量 HKT）／ADR-41（推論予算）。
- 「RBS とどう付き合うか」← 前編 Part 7、後編 Part 6（erasure）。**読むなら**
  ADR-14（sig-gen）／ADR-25・32（プラグイン RBS・inline RBS）。
- 「フロー事実をどう持つか」← 後編 Part 5（FactStore）。**読むなら**
