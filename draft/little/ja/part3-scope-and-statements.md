...
> 本書『型システムのしくみ』3・4 章に対応します。あの本は型環境を `tyEnv`（変数名→型の対応）
...
返します。本書が `tyEnv` を破壊せず `{ ...tyEnv, x: 型 }` とコピーしたのと同じ作法で、これを
オブジェクトの形にしました。

- **① 型理論**：変数の型を覚える対応＝型環境 tyenv（本書 3・4 章）。
...
- **① 型理論**：文を順に評価し、環境を育てながら進む（本書 4 章の逐次実行）。
...
本書はここで「同一ブロックでの再定義をエラーにするか」を論点にしましたが、学習の本筋では
...
| | 内容 |
|---|---|
| ① 型理論（本書 3・4 章） | 変数の型を覚える型環境 tyenv、文を縫う逐次実行 |
