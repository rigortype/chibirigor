...
> 副読本『型システムのしくみ』9 章「ジェネリクス」の*型代入*が遠い親戚ですが、ここで本当に
...
- **① 型理論**：宣言された型を引いて使う（副読本 9 章の型代入の遠縁）。
...
| | 内容 |
|---|---|
| ① 型理論（副読本 9 章） | 宣言された型を引いて使う（型代入の遠縁） |
...
- **型変数の置換**（`Array[Elem]` に `String` を入れて `Array[String]`）。副読本 9 章の `subst` の
  正道版。
