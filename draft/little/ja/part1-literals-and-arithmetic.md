...
>
> （このコラムは他言語に詳しい人向けの寄り道です。TypeScript などを知らなくても本文の理解には
> 影響しません ― ピンと来なければ読み飛ばして構いません。）
>
...
>
> ※ ここでの `Dynamic[Top]` は、本文で作っている `Dynamic`（`Dynamic.new`）と**同じもの**です
> （`[Top]` は「中身は何でもありうる」を明示した内部表記。本文の最小版では添えていないだけ）。
>
...
> 取るか、で態度が真逆なのが要点です。
>
> なお、この「チェッカーに黙れと告げる」家族には**戻り値の型**版もあります ― `void`（「値は
> 返るが見るな」）です。`unknown`（値の中身が不明）とも `never`／`Bot`（そもそも返らない。Part 4）
> とも違う第三のものなので、戻り型を扱う Part 8 のコラムでまとめて並べます。
...
> RBS の境界で**粗い型に落とした**後の保守的な型（`String`）の 2 つです。
>
> 「なぜ `annotate` の出力とシグネチャが違うのか」を調べるときに使います。
> chibirigor の `annotate` は内部型だけを見せますが、実ツールでは
> 「内部では精密に知っているが、境界では捨てる」という二重構造があるわけです。
>
> この「精密な内部型を、RBS で表せる粗い型に落とす」境界の操作を、Rigor は **erasure** と
> 呼びます。名前は似ていますが **Java ジェネリクスの「型消去（type erasure）」とは別物**です ―
> あちらは*実行時に*型引数 `<String>` を消す話、こちらは*静的な*型の精度を境界で丸める話。
> 仕組みは後編で扱います（用語集にも一行）。
