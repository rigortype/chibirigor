...
module Chibirigor
  ...
  def type_of_call(node, diagnostics)
    ...
    if node.name == :+ && integerish?(recv)
      ...
      # ★ ここがポイント：両方が既知値の Const なら *実際に計算して畳む*。
      #   大きすぎ（予算超過）や変数混じりなら、Integer に「丸める」。
...
- では `type_of(1 + 2)` は？ ── 両方が*既知値*の `Const` なら、**実際に足して `Const[3]` に
  畳めます**。「値そのもの」をもう一段保てる（実 Rigor の `Constant<3>` リテラル精度の縮図）。
- でも `type_of(x + 2)` は？ `x` が変数で値が分からなければ畳めない ── そこは **`Integer` に
  丸めます**。さらに `100000 * 100` のように*大きくなりすぎ*たら、畳まず丸める（`Const` が際限なく
  増えるのを防ぐ＝**予算**）。
- この「**畳めれば畳む・無理なら丸める**」の境目が、実 Rigor では『正規化』と『推論予算』という
  大きなテーマになります。いまは「**畳めれば値、ダメなら `Integer`**」と覚えれば十分。深掘りは
  続編で。
...
`42` は `Const[42]` なので `42` と細かく出る。`1 + 2` は両方が既知値なので**畳めて `3`**。`foo.bar` は
...
| ③ Rigor 実装の問題 | 値そのものを型にする（`Const[1]`）と細かい。*畳めれば畳む・無理なら丸める*の境目＝正規化・予算（続編で深掘り） |

## 演習

1. `annotate("3.14")` と `annotate("true")` の結果を確かめ、`Const#to_s` がどう効いているか
   説明せよ。
2. いまの `type_of_call` は `+` だけを畳む。`1 - 2` も畳めるよう拡張し、`annotate("10 - 4")` が
   `6` になることを確かめよ（ヒント：条件を `:+`/`:-` の両方に）。
3. `annotate("100000 * 100")` が畳まれず `Integer` に丸められる理由（予算超過）と、
   `annotate("1 + 2 + 3")` が `6` に畳まれる理由（再帰で両オペランドが `Const`）を説明せよ。
