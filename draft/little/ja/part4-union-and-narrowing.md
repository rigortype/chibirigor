...
> **コラム：到達できない枝の「型」は何か？ ― ボトム型と `never`**
>
> （このコラムは型の理屈に踏み込む寄り道です。読めなくても本文の実装には影響しません ―
> 難しければ飛ばして構いません。）
>
> 前編 Part 1 で**トップ型**（`Top`／`unknown`、「何でもありうる」格子のてっぺん）を見ました。
> その**真逆**にあるのが**ボトム型**（`Bot`、⊥）です。トップが「何でも入る一番大きい箱」なら、
> ボトムは「**値が一つも入らない一番小さい箱**」 ― *到達できない・起き得ない*ことを表す型です。
>
> 上で見た dead branch（`x : Integer` に `if x.is_a?(String)` を当てた then 枝）が、まさに
> これ。その枝で `x` がもし型を持つなら「Integer かつ String」 ― そんな値は**存在しない**ので、
> 型は空集合 ＝ ボトムになります。「絞り込んで候補がゼロになった」状態の型、とも言えます。
>
> | 言語 / ツール | 名前 | どこに現れるか |
> |---|---|---|
...
>
> 性質もトップと**双対**です。トップ型の値は「何でもありうるので**絞るまで何もできない**」。
> 逆にボトム型の値は「**そもそも手に入らない**ので、手に入ったと仮定すれば**何にでも使える**」
> （`Bot <: T`：どんな型の部分型でもある ― 後編 Part 2 の格子図）。`never` を返す関数 ＝
> 「絶対に戻ってこない」関数、というのはこの言い換えです（`raise` だけして値を返さない経路）。
>
> chibirigor 本体ではボトム型を*型としては*作らず、「到達できない枝」を**診断**として
> 扱うにとどめます（実 Rigor の unreachable arm、ADR-47）。「脅かさない」方針では、
> 空集合を律儀に伝播させるより「ここは通らないよ」と一言伝える方が現場で役に立つからです。
