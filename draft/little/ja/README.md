...
- **後編 = The Seasoned chibirigor** … 高度な型理論・作り込み（`../../seasoned/ja/`、ドラフト済み）。

> はじめての方は二巻共通の [**まえがき**](../../preface.md)（動機・対象読者・前提・環境構築）から。
> 用語は巻末の [**用語集**](../../glossary.md) で引けます。各章末に**演習**があります。
...
各章には **参考書メモ**（任意）として、TAPL（Pierce『型システム入門 ― プログラミング言語と
型の理論』）と『しくみ』（遠藤『型システムのしくみ』）の対応章を併記します ― どちらも必読では
ありません（Part 0 参照）。英語版へ移植する際は TAPL が共通の参照先になります。
