...
> 実装）も同じく部分型です。
>
> でも**部分型は継承・実装だけではありません**。Part 5 で「キーが*多い*ハッシュは、キーが*少ない*
...
> **Rigor／chibirigor は両方を『箱に入るか』という一つの判定にまとめて扱う**。`accepts` は
> その入口です（名前的・構造的の使い分けの形式は後編 Part 2 で）。
...
後編 Part 2 §2-3a で、この原則と型理論の「変性（S-Arrow 規則）」と「LSP（オブジェクト指向の
置換可能性原則）」が全く別の出発点から**同じ規則**に至ることを確認します。
