...
> 副読本『型システムのしくみ』では 7 章「部分型付け」が対応します。あの本は「合う／合わない」を
...
これが**受理判定**です。副読本 7 章ではこれを `subtype(a, b)`（a は b の部分型か）として作り、
...
この「箱に入るか」が、副読本 7 章の**部分型付け**そのものです（用語は覚えなくて大丈夫。
*小さい箱は大きい箱に入る*、とだけ）。副読本は箱の大小を丁寧に階段状に定義しましたが、私たちは
まず「クラスが同じか」という一番素朴な所から始めます。

- **① 型理論**：値が期待の型に*入る*か＝部分型付け（副読本 7 章）。
...
副読本 7 章にも、よく似た問題意識のコラムがありました。「健全な型システムが*良いプログラムを
弾く*のは誤検知だ」と。副読本は誤検知を*減らす*方向で工夫しました。Rigor はもう一歩進めて、
...
- 箱の大小をきちんと階段状に定義する**本物の部分型**（副読本 7 章の width/depth）。
- **変性**（関数を渡すとき、引数の向きだけ逆になる「反変」）。副読本 7 章の山場。
