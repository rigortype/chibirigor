...
> 検証済み）。Part 5 の山は「open か closed か」＝『しくみ』とちょうど*逆向き*の設計判断を、やさしく
...
> 『しくみ』5 章「オブジェクト型」に対応します。あの本は同じものを
...
| | 内容 |
|---|---|
| ① 型理論（『しくみ』 5 章） | 値をラベルでまとめる＝レコード型。キーが多い方が部分型 |
...
> **検証メモ（この試し書きの自己評価）**
> - 中難度の山「open か closed か」は、『しくみ』との*逆向き対比*として書くと、かえってやさしく
>   なった（「『しくみ』は NG、Rigor は OK、なぜなら Ruby の現実」）。対比が説明の梃子になる。○
