...
- **v1 清書版 = `v1/ja/`（draft の外）**：原稿 `draft/` を**二巻維持・大胆再構成**で再編した清書ツリー（2026-06-08 着手）。
  ...
  - **6レンズ査読1巡 実施済み（2026-06-08）**：編集者・型理論・フィデリティ・校閲・Java読者・Ruby読者。
    ...
    refinement carrier 二層注記・読者高一致の一文×5・用語集初出タグ）。**未適用バックログ**：
    編集者 C1（Part4/5 演習の重複一本化）・C3（Part4/7 fuel表の二重）・C4（用語集↔a2 PHPStan表の二重保守）・
    C5（Part6 冒頭の反復圧縮）／前編3図の本清書／後編README↔§1-6 地図の二重／「地に足」比喩の間引き。
